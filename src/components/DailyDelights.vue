...
<template>
  <section class="h-full w-full bg-background p-4 text-foreground">
    <div class="mb-4 flex items-center gap-2"><Sparkles class="size-8 text-amber-500" /><h2 class="text-4xl font-bold tracking-tight">Daily delights</h2></div>
    ...
    <div v-else-if="daily" class="grid h-[calc(100%-3rem)] grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <div class="grid min-h-0 grid-rows-2 gap-4">
        <Card><CardHeader class="pb-2"><CardTitle class="flex items-center gap-2 text-3xl"><Quote class="size-9 text-amber-500" />Quote of the day</CardTitle></CardHeader><CardContent><blockquote class="text-5xl font-medium leading-snug">“{{ daily.quote.text }}”</blockquote><p class="mt-3 text-3xl text-muted-foreground">— {{ daily.quote.author }}</p><p class="mt-4 text-xl text-muted-foreground">Quotes provided by ZenQuotes</p></CardContent></Card>
