...
import { AlertTriangle, Droplets } from '@lucide/vue'
...
function alertStyle(severity: string) {
  return ['Extreme', 'Severe'].includes(severity)
    ...
    : 'border-kiosk-orange bg-orange-100 text-orange-950 dark:border-amber-400 dark:bg-amber-400/15 dark:text-amber-50'
}
      ...
      <div class="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card class="min-h-0 lg:col-span-2"><CardHeader class="pb-2"><CardTitle class="text-3xl">Next 12 hours</CardTitle></CardHeader><CardContent class="grid h-[calc(100%-3.5rem)] min-h-0 grid-cols-12 gap-2 px-4 pb-4">
          <article v-for="hour in weather.hourly" :key="hour.time" class="flex min-w-0 flex-col overflow-hidden rounded-md bg-muted/40 p-2 text-center">
            ...
            <div class="mt-2 flex flex-col items-center justify-center gap-1"><component :is="weatherIcon(hour.weatherCode)" class="size-10 shrink-0 text-sky-700 dark:text-sky-300" /><span class="text-4xl font-semibold">{{ Math.round(hour.temperature) }}°</span></div>
            <div class="mt-1 flex items-center justify-center gap-1 text-2xl font-semibold text-sky-700 dark:text-sky-300"><Droplets class="size-6" /><span>{{ Math.round(hour.precipitationProbability) }}%</span></div>
          </article>
        </CardContent></Card>
        ...
        <Card class="lg:col-span-3"><CardHeader class="pb-2"><CardTitle class="text-3xl">Five-day forecast</CardTitle></CardHeader><CardContent class="grid grid-cols-5 gap-3 pb-5">
          <div v-for="day in weather.daily" :key="day.date" class="flex flex-col items-center gap-3 rounded-md bg-muted/40 p-4 text-center"><span class="text-3xl font-medium">{{ dayjs(day.date).format('ddd') }}</span><component :is="weatherIcon(day.weatherCode)" class="size-20 text-kiosk-orange dark:text-amber-400" /><span class="text-4xl font-semibold">{{ Math.round(day.high) }}° <span class="font-normal text-muted-foreground">{{ Math.round(day.low) }}°</span></span></div>
        </CardContent></Card>
      </div>
    </template>
  </section>
